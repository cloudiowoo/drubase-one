...
for i in {1..12}; do
    echo "   尝试连接数据库 ($i/12)..."
    if docker-compose -f docker/docker-compose.yml exec -T pg17 pg_isready -U postgres -d drubase >/dev/null 2>&1; then
        echo "✅ 数据库已准备就绪"
        break
    fi
    if [ $i -eq 12 ]; then
        echo "❌ 数据库启动超时，请检查日志: docker-compose -f docker/docker-compose.yml logs pg17"
        exit 1
    fi
    sleep 10
done
...
echo "⚙️  准备 Drupal 安装..."
echo "   设置文件权限..."
docker-compose -f docker/docker-compose.yml exec -T php8-4-fpm bash -c "cd /var/www/html && \
...
echo ""
echo "💡 提示: 如果遇到权限问题，请手动设置 docker/pg/v17/log 目录权限"
echo ""
