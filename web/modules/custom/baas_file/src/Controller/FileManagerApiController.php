...
class FileManagerApiController extends ControllerBase {
  ...
  public function getProjectMediaList(string $tenant_id, string $project_id, Request $request): JsonResponse {
    ...
    try {
      ...
      $query->fields('pfa', ['project_id', 'timestamp', 'action', 'user_id']);
      $query->condition('pfa.project_id', $project_id);
      $query->condition('f.status', 1); // 只显示永久文件
      $query->orderBy('f.changed', 'DESC');
      ...
      foreach ($files as $file) {
        $formatted_files[] = [
          ...
          'size_formatted' => $this->formatFileSize((int) $file->filesize),
          'is_image' => strpos($file->filemime, 'image/') === 0,
          'created' => (int) $file->created,
          'changed' => (int) $file->changed,
          'project_id' => $file->project_id,
          'tenant_id' => $tenant_id,
          'uploaded_timestamp' => (int) $file->timestamp,
          'uploaded_by' => (int) $file->user_id,
        ];
      }
      
      // 获取总数用于分页
      $count_query = $this->database->select('file_managed', 'f');
      $count_query->innerJoin('baas_project_file_access', 'pfa', 'f.fid = pfa.file_id::integer');
      $count_query->condition('pfa.project_id', $project_id);
      $count_query->condition('f.status', 1);
  ...
  public function getFileDetail(string $tenant_id, string $project_id, int $file_id, Request $request): JsonResponse {
    ...
    try {
      ...
      $query->innerJoin('baas_project_file_access', 'pfa', 'f.fid = pfa.file_id::integer');
      $query->fields('f', ['fid', 'filename', 'uri', 'filemime', 'filesize', 'created', 'changed']);
      $query->fields('pfa', ['project_id', 'timestamp', 'action', 'user_id', 'ip_address']);
      ...
      if ($file->user_id) {
        ...
        $uploader_user = $user_storage->load($file->user_id);
        if ($uploader_user) {
      ...
      $file_detail = [
        ...
        'uploaded_by_id' => (int) $file->user_id,
        'project_id' => $file->project_id,
        'tenant_id' => $tenant_id,
        'upload_ip' => $file->ip_address,
        'created_formatted' => date('Y-m-d H:i:s', (int) $file->created),
        'changed_formatted' => date('Y-m-d H:i:s', (int) $file->changed),
        'uploaded_formatted' => date('Y-m-d H:i:s', (int) $file->timestamp),
      ];
