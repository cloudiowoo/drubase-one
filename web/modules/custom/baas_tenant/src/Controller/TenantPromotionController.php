  ...
  public function promoteUserToTenant(UserInterface $user): RedirectResponse {
    try {
      ...
      if ($this->isUserTenant($user)) {
        ...
        if ($tenant_id) {
          $this->messenger->addStatus($this->t('用户 @username 已成功提升为项目管理员。租户 @tenant_id 已自动创建。', [
            '@username' => $user->getAccountName(),
            '@tenant_id' => $tenant_id,
          ]));
        } else {
          $this->messenger->addStatus($this->t('用户 @username 已成功提升为项目管理员。', [
            '@username' => $user->getAccountName(),
          ]));
        }

        // 记录操作日志
        \Drupal::logger('baas_tenant')->info('管理员 @admin 将用户 @user (UID: @uid) 提升为项目管理员，租户ID: @tenant_id', [
